...
class myClassifier:
    ...
    def load_data(self):
        ...
        # write feature names all drop
        with open('feature_names_after_drop.pkl', 'wb') as fo:
            pickle.dump(new_feature_names, fo)
        ...
        raw_data.to_csv('processed_features.csv', index=False)
        raw_labels.to_csv('labels.csv', index=False)
        '''
        
        raw_data = pd.read_csv('processed_features.csv', index_col=None)
        raw_labels = pd.read_csv('labels.csv', index_col=None, header=None)
        raw_labels = raw_labels[0]
        
        # load top feature list
        with open('top_features.pkl', 'rb') as fi:
            top_feature_names = pickle.load(fi)
        
        raw_data = raw_data[top_feature_names]
        
        # split data into training and testing sets
        self.training_features, self.testing_features, self.training_labels, self.testing_labels = train_test_split(raw_data, raw_labels,test_size=0.2,
                                                                                                          random_state=6)
    ...
    def model_training(self, params, training_features, training_labels, validation_features, validation_labels):
        ...
        model.save_model('sd_crd_ver1.m')
    ...
    def hyper_optimization(self):
        ...
        best_sln['best_boost_num'] = best_boost_num
        with open('sd_bst_param_set.pkl', 'wb') as fo:
...
if __name__ == '__main__':
    ...
    model.save_model('crd_sd_ver1.m')
...
#    model = xgb.Booster(model_file='crd_ver1.m')
